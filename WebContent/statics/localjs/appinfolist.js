 $("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		...
		$.ajax({
			"type":"GET",//请求类型
			"url":"/appinfo/appCategory/categorylevellist",//请求的url
...
$(".addVersion").on("click",function(){//新增版本
	var obj = $(this);
	window.location.href="/appinfo/appVersion/appversionadd?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){//修改版本
	var obj = $(this);
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/appinfo/appVersion/appversionmodify?vid="+ versionid + "&aid="+appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){//修改
	var obj = $(this);
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/appinfo/appinfo/appinfomodify?id="+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	if(obj.attr("aa")=="上架"){
		$.ajax({
			type:"get",
			url:"/appinfo/appinfo/sale.json",
			data:{id:appId,status:"4"},
			dataType:"json",
...
$(".viewApp").on("click",function(){//查看
	var obj = $(this);
	window.location.href="/appinfo/appinfo/appview/"+ obj.attr("appinfoid");
});
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:"/appinfo/appinfo/delapp.json",
