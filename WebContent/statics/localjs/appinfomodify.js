function  loadCategoryLevel(pid,cl,categoryLevel){
	$.ajax({
		type:"GET",//请求类型
		url:"/appinfo/appCategory/categorylevellist.json",//请求的url
		data:{pid:pid},//请求参数
...
function  loadCategoryLevel23(pid,cl,categoryLevel){
	$.ajax({
		type:"GET",//请求类型
...
function delfile(id){
	...
	$.ajax({
		type:"GET",//请求类型
		url:"/appinfo/appinfo/delfiletup.json",//请求的url
		data:{id:id},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
...
$(function(){  
	...
	$.ajax({
		type:"GET",//请求类型
		url:"/appinfo/dataDic/datadictionarylist.json",//请求的url
	...
	//动态加载二级分类列表
	loadCategoryLevel23(cl1,cl2,"categoryLevel2");
	//动态加载三级分类列表
	loadCategoryLevel23(cl2,cl3,"categoryLevel3");
	
	//联动效果：动态加载二级分类列表
	$("#categoryLevel1").change(function(){
		var categoryLevel1 = $("#categoryLevel1").val();
		if(categoryLevel1 != '' && categoryLevel1 != null){
			loadCategoryLevel2(categoryLevel1,cl2,"categoryLevel2");
		}else{
			$("#categoryLevel2").html("");
			var options = "<option value=\"\">--请选择--</option>";
	...
	$("#categoryLevel2").change(function(){
		...
		if(categoryLevel2 != '' && categoryLevel2 != null){
			loadCategoryLevel3(categoryLevel2,cl3,"categoryLevel3");
		}else{
			$("#categoryLevel3").html("");
			var options = "<option value=\"\">--请选择--</option>";
			$("#categoryLevel3").html(options);
